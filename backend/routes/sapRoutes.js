...
router.get("/batch/300/:batchNumber", async (req, res) => {
  try {
    ...
    const url = `${API_URL}?$filter=BatchNumber eq '${batchNumber}'`;
...
router.get("/BatchInfoGateway/:batchNumber", async (req, res) => {
  try {
    ...
    if (environment === "dev" || environment === "110") {
      // Use API Management URL for dev
      const filter = `Charg eq '${batchNumber}' and Werks eq '1134'`;
      url = `${API_URL}/BatchInfoSet?$filter=${encodeURIComponent(filter)}&$format=json&sap-client=110`;
    } else {
      // Use production URL for prd/300
      url = `${API_URL}?$filter=Charg eq '${batchNumber}'`;
    }
