...
function BspPage2({ user, onLogout }) {
  ...
  useEffect(() => {
    ...
    navigate('/bsp2', { replace: true, state: Object.keys(nextState).length ? nextState : null });
  ...
  const next = () => {
    ...
    navigate("/migo2", { state: { batchData: batchListToSend } });
  };
      ...
      {showClearConfirm && (
        <div style={{
          ...
          <div style={{
            ...
            <h3 style={{ margin: '0 0 1rem 0', color: '#333' }}>Confirm Clear All</h3>
            <p style={{ margin: '0 0 1.5rem 0', color: '#666' }}>
              Are you sure you want to clear all batch entries? This action cannot be undone.
            </p>
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
              <button
      ...
      <div style={{ maxWidth: "600px", margin: "20px auto", padding: "1rem" }}>
        <div style={{ background: "white", borderRadius: "12px", padding: "1.5rem", boxShadow: "0 4px 12px rgba(0,0,0,0.06)" }}>
          ...
          {success && (
            ...
            />
            <button onClick={fetchBatch} disabled={loading} style={{ padding: "0.85rem 1.5rem", background: "#3b82f6", color: "#fff", border: "none", borderRadius: "8px", verticalAlign: "top", textAlign: "left" }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <tbody>
                  {Object.entries(cleanBatch(selectedBatch))
                    .filter(([key]) => key !== 'SalesOrder' && key !== 'SOBKZ' && key !== 'SoItem')
                    .map(([key, value]) => (
